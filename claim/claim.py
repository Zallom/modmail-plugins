...
class ClaimThread(commands.Cog):
    ...
    async def addclaim(self, ctx, *, member: discord.Member):
        ...
        if thread and (str(ctx.author.id) in thread['claimers'] or checks.has_permissions(PermissionLevel.MODERATOR)(ctx)):
...
async def check_reply(ctx):
    ...
    if thread:
        return ctx.author.bot or str(ctx.author.id) in thread['claimers'] or checks.has_permissions(PermissionLevel.MODERATOR)(ctx)
    return True
